...
| **Facturation Stripe (V3)** | **Freemium** : plan gratuit permanent (aucune limite de durée) plafonné à 25 personnes, et **un seul plan payant** à 39 €/mois (`apps.billing.Subscription`, 1:1 avec `Organization`). Le prix facturé est configuré côté Stripe Dashboard (`STRIPE_PRICE_ID` référence un Price Stripe), jamais en dur dans le code — le prix affiché sur `/settings/billing/` doit rester cohérent avec lui. Stripe Checkout pour la souscription, Stripe Customer Portal pour la gestion (résiliation, moyen de paiement) — FitRadarHR ne ré-implémente aucun flux de paiement lui-même. Le statut (`free`/`active`/`past_due`/`canceled`) est synchronisé exclusivement via un **webhook signé** (`/billing/webhook/`, hors `i18n_patterns`), jamais interrogé en direct auprès de Stripe à chaque page vue ; un statut Stripe `trialing` est traité comme `active`. Sans abonnement actif, un **seuil unique et cumulé** s'applique : 25 personnes au total dans l'organisation (`apps/billing/quotas.py`) — postes et questionnaires restent libres, le nombre de personnes capturant implicitement le volume d'usage. Jamais de blocage total, uniquement l'ajout de nouvelles personnes. L'org de démonstration (`is_demo`) est toujours exemptée. (Le modèle initial — essai 14 jours puis quotas — a été remplacé par ce freemium le 2026-07-06.) |
