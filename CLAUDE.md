...
- [x] Mesure d'audience — Matomo self-hosted (`/srv/matomo`, `deploy/matomo/docker-compose.yml`), sous-domaine `analytics.fitradarhr.fr` (Caddy), tracker cookieless dans `base.html` (désactivé en mode démo, hook HTMX pour les navigations partielles), exemption de consentement CNIL (checklist de configuration + funnels questionnaire/poste/équipe dans `docs/technical/analytics.md`), section « Mesure d'audience » ajoutée à la politique de confidentialité (E8)
- [x] Cartographie des manques d'équipe (roadmap V3 #3, US-E6-08) — rapport dédié `/reports/team/<uuid>/gaps/` (sans personne à sélectionner), réutilise `compute_team_profile`/`HOMOGENEITY_STD_THRESHOLD` du moteur de fit (E5) sans nouveau calcul, 5 dimensions toujours affichées + section "pistes à explorer" conditionnelle sur les dimensions homogènes, export PDF, audit log, doc utilisateur `docs/user/reports/team-gaps.md` (E6)
- [x] Fondations SEO multilingues — s'appuie sur `i18n_patterns()` déjà en place (une URL par langue : `/`, `/en/`, `/es/`, `/de/`) : `core/context_processors.py::seo` calcule les URLs alternates via `translate_url()`, `templates/base.html` expose canonical/hreflang/meta description/Open Graph/Twitter card (tag `{% capture %}` maison dans `apps/accounts/templatetags/seo_tags.py` pour réutiliser title/description sans dupliquer les blocs), `core/seo.py` génère `robots.txt` (disallow des zones privées par préfixe de langue) et `sitemap.xml` (3 pages publiques × 4 langues avec hreflang alternates). `privacy_policy` rendue publique (elle était par erreur `@login_required`). 14 tests (`core/tests.py`)
